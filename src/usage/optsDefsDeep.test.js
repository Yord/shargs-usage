const {optsDefsDeep, optsDefsDeepFrom} = require('..')
...
test('optsDefsDeep works with all option types', () => {
  const varOpts = [
...
test('optsDefsDeep prints an empty string if opts are undefined', () => {
  const style = {
    line: {width: 40},
    desc: {padStart: 4, width: 36}
  }
  
  const res = optsDefsDeep()(style)
...
test('optsDefsDeepFrom correctly passes on ids', () => {
  const varOpts = [
    {key: 'variadicPos', desc: 'Variadic.' },
  ]
  ...
  const res = optsDefsDeepFrom('line2', 'desc2')(opts)(style)
...
test('optsDefsDeepFrom uses line and desc if no ids are defined', () => {
  const varOpts = [
    {key: 'variadicPos', desc: 'Variadic.' },
  ]
  ...
  const res = optsDefsDeepFrom()(opts)(style)
